...
def RedirectUser(strPostData):
    return RedirectResponse(url="{0}?{1}".format(_provider, strPostData),
                            status_code=HTTP_303_SEE_OTHER,
                            headers={'Content-Type': 'application/x-www-form-urlencoded'})

def ConstructURL(responseURL) -> str:
...
def ValidateResults(results):
    ...
    with urllib.request.urlopen(_provider, parsedArgs) as requestData:
